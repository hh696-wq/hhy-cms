...
当前版本：**0.1.2**。新增可搜索媒体选择器与所见即所得编辑器，包含站点设置换行修复。
...
./bin/hhy-web check
./bin/hhy-web test
# 可选：开发环境使用 Node.js 检查编辑器序列化
node tests/editor-serialization.cjs
```
...
封面和 Logo 通过可搜索、分页的媒体选择器管理，可在当前编辑页直接上传 PNG/JPEG（最大 2 MiB），上传后自动选中并预览，再保存表单使其生效。
正文使用本地 Quill 2.0.3 所见即所得编辑器，支持标题、加粗、斜体、列表、引用及撤销重做。资源与 BSD 许可证位于 `public/vendor/quill/`，无需 CDN 或前端构建；服务端继续按无属性标签白名单过滤正文。编辑器加载失败时保留 HTML 文本框。
